...
class TestSettings:
    ...
    def test_default_claude_workspace_uses_cfc_home(self, monkeypatch, tmp_path):
        ...
        assert default_claude_workspace_path() == tmp_path / ".cfc" / "agent_workspace"
        assert not hasattr(settings, "claude_workspace")

    def test_server_log_path_uses_cfc_home(self, monkeypatch, tmp_path):
        """The server log location is fixed under ~/.cfc."""
        from config.paths import server_log_path

        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("USERPROFILE", str(tmp_path))

        assert server_log_path() == tmp_path / ".cfc" / "logs" / "server.log"
    ...
    def test_blank_claude_workspace_uses_cfc_home(self, monkeypatch, tmp_path):
...
class TestPerModelMapping:
    ...
    def test_configured_chat_model_refs_collects_unique_models_with_sources(
        ...
        monkeypatch.setenv("CFC_SMOKE_MODEL_NVIDIA_NIM", "nvidia_nim/smoke")
